...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
    <div class="form-group">
        <label for="query" class="control-label">Nhập mã lớp:</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($xml !== false) {
        ...
        foreach ($xml->student as $student) {
            if (strpos(strtolower($student->class_code), strtolower($query)) !== false) {
                $found_students[] = $student;
            }
        }

        if (!empty($found_students)) {
            echo "<h3>Danh sách sinh viên lớp $query:</h3>";
            ...
            echo "Không tìm thấy sinh viên trong lớp $query.";
        }
    } else {
